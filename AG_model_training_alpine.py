...
class WeatherForecaster:
    ...
    def _load_data(self) -> TimeSeriesDataFrame:
        ...
        static_df = pd.DataFrame({
            'item_id': self.config['valley_ids']
        })
    ...
    def _generate_future_covariates(self, last_date: pd.Timestamp,
    ...
    def _update_dataset(self, history: TimeSeriesDataFrame,
                      forecast: TimeSeriesDataFrame) -> TimeSeriesDataFrame:
        """Append forecasts to training data for next iteration"""
    ...
    def _plot_predictions(self, forecasts: Dict[str, pd.DataFrame]):
        ...
        for valley_id, data in forecasts.items():
            ...
            plt.plot(hist['timestamp'], hist[self.config['target']],
                    color=colors[valley_id], linestyle='--',
                    label=f'{valley_id} Actual')

            # Plot forecasts
            fcst = data.reset_index()
            plt.plot(fcst['timestamp'], fcst['0.5'],
                    color=colors[valley_id],
                    label=f'{valley_id} Forecast')

            # Confidence interval
            plt.fill_between(fcst['timestamp'], fcst['0.1'], fcst['0.9'],
                            color=colors[valley_id], alpha=0.2,
                            label=f'{valley_id} 80% CI')

        plt.title(f"{self.config['target'].title()} Forecast until {self.config['target_year']}")
        plt.xlabel("Year")
    ...
    def run_pipeline(self):
        ...
        while True:
            ...
            prediction_window = min(self.config['max_prediction_window'], remaining_years)
        ...
        self._plot_predictions({
            vid: self.full_history.loc[vid]
            for vid in self.config['valley_ids']
        })
        return self.full_history

# Configuration Template
CONFIG = {
    "data_path": "data/maurienne_valley_20_years_daily_data.csv", "date_column": "Date",
